...
class RegisterForm(FlaskForm):
    ...
    register_password = PasswordField(u'Password', id="regpassword", validators=[DataRequired("Please add a password"), Length(min=2, max=40)])
...
class LoginForm(FlaskForm):
    ...
    login_email = EmailField(u'Email', id="login-email", validators=[DataRequired(message="Should be a working email"), Length(min=2,max=120)])
    ...
    login_password = PasswordField(u'Password', id="logpassword", validators=[DataRequired("Please add a password"), Length(min=2, max=40)])
